...
# just fit the reciprocal mean-CV2 relationship directly in the model, rather
# than trying to necessarily linearlly regress out the relationship?
...
mesc.glm.form <- as.formula(paste("CV2",
...
tc_cpg <- ggplot(mesc.match, aes(x=CpGisland, y=CV2, colour=CpGisland)) + 
  theme_mike() + 
...
tc_cpgMe <- ggplot(mesc.match, aes(x=CpGisland, y=Reciprocal.Mean, colour=CpGisland)) + 
  theme_mike() + 
  geom_jitter(position=position_jitterdodge(jitter.height=0,
                                            jitter.width=1.5),
              alpha=0.7) +
