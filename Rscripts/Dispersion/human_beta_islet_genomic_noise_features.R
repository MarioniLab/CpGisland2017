...
set.seed(42)
panc.svm <- svm(CountVar ~ CountMean, beta.gene.summary)
beta.gene.summary$Alpha <- residuals(panc.svm)

## calculate the residual overdispersion, removing any remaining mean effect
panc.loess <- loess(Alpha ~ Mean, data=beta.gene.summary, span=0.2)
beta.gene.summary$Alpha_r <- residuals(panc.loess)
