...
# need to treat serum and 2i separately
mesc <- "~/Dropbox/mESC/Teichmann-feature_counts.tsv"
mesc.df <- read.table(mesc, sep="\t", h=T, stringsAsFactors=F)
rownames(mesc.df) <- mesc.df$gene_id
mesc.df <- mesc.df[, -1]
mesc.df <- mesc.df[, !grepl(colnames(mesc.df), pattern="bulk")]

colnames(mesc.df) <- gsub(colnames(mesc.df), pattern="\\.star\\.dedup", replacement="")

# mesc.meta.2i <- read.table("~/Dropbox/mESC/E-MTAB-2600.sdrf.txt",
#                         sep="\t", stringsAsFactors=F, h=T)
# 
# samp.stub <- unlist(lapply(strsplit(mesc.meta.2i$Scan.Name, fixed=T, split="."),
#                            FUN=function(X) paste0(X[1])))
# samp.name <- unlist(lapply(strsplit(samp.stub, fixed=T, split="_"),
#                            FUN=function(X) paste(X[1:4], collapse="_")))
# 
# mesc.meta.2i$Sample <- samp.name
# mesc.meta.2i <- mesc.meta.2i[!duplicated(mesc.meta.2i$Sample), ]

mesc.meta <- data.frame(cbind(colnames(mesc.df), 
                              unlist(lapply(strsplit(colnames(mesc.df), fixed=T, split="_"),
                                            FUN=function(X) paste0(X[2])))))
colnames(mesc.meta) <- c("Sample", "Condition")

write.table(mesc.meta,
            file="~/Dropbox/mESC/Teichman-Serum_2i-meta.tsv",
            quote=F, sep="\t")
...
spikes <- grepl(rownames(mesc.nz), pattern='ERCC')

cell_sparsity <- apply(mesc.nz == 0, MARGIN = 2, sum)/dim(mesc.nz)[1]
keep_cells <- cell_sparsity <= 0.70
...
sce <- computeSumFactors(sce, sizes=c(45, 50, 55, 60), positive=TRUE,
                         assay='counts', clusters=clusters, get.spikes=TRUE)
summary(sizeFactors((sce)))
hist(sizeFactors((sce)), 100)
sce <- normalize(sce)

sf.norm <- data.frame(exprs(sce))
sf.norm$gene_id <- rownames(sf.norm)
write.table(sf.norm,
            file="~/Dropbox/mESC/Teichman-Serum_2i-mESC_SFnorm.tsv",
...
write.table(rownames(hvg.df),
            file = "~/Dropbox/mESC/Teichman-Serum_2i-mESC_HVG.tsv",
            sep="\t", row.names=F, quote=F)

mesc.hvg <- mesc.nz[HVG, ]

# cluster cells on the basis of their HVGs
mesc.cor <- cor(mesc.hvg, method='spearman')
...
# plot!
mesc_t <- ggplot(mesc.uber, aes(x=Dim1, y=Dim2, colour=Condition)) + 
  geom_point(size=1) + theme_mike() +
  scale_colour_Publication()
mesc_t

ggsave(mesc_t, filename='~/Dropbox/mESC/plots.dir/Teichman_mESC_2i_serum-RtSNE-HVGs.png',
